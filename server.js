...
const APPLE_PAY_CONFIG = {
  ...
  merchantIdentifier: process.env.APPLE_MERCHANT_ID || 'merchant.com.yourcompany.app',
  
  // 你的域名（例如：pay.yourdomain.com）
  domainName: process.env.APPLE_DOMAIN || 'pay.yourdomain.com',
  
  // 显示名称
  displayName: process.env.APPLE_DISPLAY_NAME || 'Your Store',
  
  // Apple 验证服务器地址
  appleValidationURL: 'https://apple-pay-gateway.apple.com/paymentservices/startSession'
};

// 证书配置（从 .p12 文件导出的证书和私钥）
// 支持两种方式：
// 1. 文件路径（本地开发）
// 2. 环境变量直接传入证书内容（Railway 等云平台）
const CERT_CONFIG = {
  // 商户身份证书路径（从 .p12 导出的证书）
  certPath: process.env.CERT_PATH || './certs/merchant_id_cert.pem',
  
  // 私钥路径（从 .p12 导出的私钥）
  keyPath: process.env.KEY_PATH || './certs/merchant_id_key.pem',
  
  // 如果有 CA 证书，可以配置（可选）
  caPath: process.env.CA_PATH || null,
  
  // 直接传入证书内容（Base64 编码）
  certBase64: process.env.CERT_BASE64 || null,
  keyBase64: process.env.KEY_BASE64 || null
};
